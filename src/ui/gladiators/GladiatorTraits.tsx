...
function TraitBadge({
  ...
  return (
    <Tooltip
      content={
        <span className="gladiator-traits__tooltip">
          <strong>{label}</strong>
          <span>{t(definition.descriptionKey)}</span>
          {definition.modifiers.length > 0 ? (
            <span className="gladiator-traits__tooltip-modifiers">
          ...
          {durationLabel ? (
            <span className="gladiator-traits__tooltip-duration">{durationLabel}</span>
          ) : null}
        </span>
      }
    >
      ...
      >
        ...
        <span>{label}</span>
        {duration ? (
          <small>{t('traits.duration.shortDays', { count: duration.days })}</small>
        ) : null}
      </span>
    </Tooltip>
  );
